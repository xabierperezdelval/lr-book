...
<!-- Code Appended by mPower Global Inc. -->
<%
	HttpServletRequest httpServletRequest = PortalUtil.getOriginalServletRequest(request);

	String inviterId = ParamUtil.getString(request, "inviterId", 
			GetterUtil.getString(httpServletRequest.getParameter("inviterId")));
	String inviteeEmail = ParamUtil.getString(request, "emailAddress", 
			GetterUtil.getString(httpServletRequest.getParameter("inviteeEmail")));
	
	boolean isInvited = (Validator.isNotNull(inviterId) && Validator.isNotNull(inviteeEmail));
...
<c:if test="<%= isInvited %>">
	<aui:script>
		AUI().ready(function() {
			...
			frm.<portlet:namespace/>emailAddress.value = '<%= inviteeEmail %>';
			frm.<portlet:namespace/>emailAddress.readOnly = true;
			
			// attaching hidden variables for inviterId
			var attrValue = document.createElement("input");
			attrValue.type = "hidden";
   			attrValue.name = "inviterId";
   			attrValue.value = "<%= inviterId %>";
   			frm.appendChild(attrValue);			
		});
 	</aui:script>
</c:if>